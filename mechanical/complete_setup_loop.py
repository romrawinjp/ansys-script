...
for i in range(len(Model.Materials.Children)):
    ...
    elif "Silicon" == Model.Materials.Children[i].Name:
...
# Mesh quality
mesh = Model.Mesh
mesh.ElementSize = Quantity("0.015 [mm]")
mesh.GenerateMesh()
...
analysis_setting.SetStepEndTime(4, Quantity("0.00003 [s]"))
...
for analysis in Model.Analyses:
	...
	for result in results:
            ...
            for time_step in range(1, len(T)):
                ...
                result.ExportToTextFile("D:\\"+result.Name+"\\"+str(time_step)+".txt")
...
for analysis in Model.Analyses:
	...
	for result in results:
            ...
            file = open("D:\\" + result.Name +"\\stress_probe.txt", "w")
...
#Loop
import os
p = ["0.1 [MPa]", "0.2 [MPa]", "0.3 [MPa]", "0.4 [MPa]", "0.5 [MPa]"]
# Set Unit System
ExtAPI.DataModel.Project.UnitSystem = UserUnitSystemType.StandardNMM
# Add-Pressure
for lp in range(len(p)):
    path = os.path.join("D:\\", p[lp])
    os.mkdir(path)
    analysis = Model.Analyses[0]
    pressure = analysis.AddPressure()
    pressure.Location = Model.NamedSelections.Children[6]
    pressure.Magnitude.Inputs[0].DiscreteValues = [Quantity("0 [s]"), Quantity("2e-5 [s]"), Quantity("2.5e-5 [s]"), Quantity("3e-5 [s]")]
    pressure.Magnitude.Output.DiscreteValues = [Quantity(p[lp]), Quantity(p[lp]), Quantity("0 [MPa]"), Quantity("0 [MPa]")]
    ...
    for analysis in Model.Analyses:
    	results = analysis.Solution.GetChildren(DataModelObjectCategory.Result, True)
    	for result in results:
                path = os.path.join("D:\\"+p[lp]+"\\", result.Name)
                os.mkdir(path)
                for time_step in range(1, len(T)):
                    print(result.Name, "______"+str(time_step)+"_______")
                    if time_step == int(Step[-1]):
                        ...
                        result.DisplayTime = Quantity(T[time_step]+" [sec]")
                    result.EvaluateAllResults()
                    result.ExportToTextFile("D:\\"+p[lp]+"\\"+result.Name+"\\"+str(time_step)+".txt")
    ...
    for analysis in Model.Analyses:
    	...
    	for result in results:
                ...
                T = intersperse(T, ",")
                stress_probe = intersperse(stress_probe, ",")
                file = open("D:\\"+p[lp]+"\\" + result.Name +"\\stress_probe.txt", "w")
                file.writelines(Step)
                file.write("\n")
                file.writelines(T)
                file.write("\n")
    ...
    path = os.path.join("D:\\"+p[lp]+"\\", "video")
    os.mkdir(path)
    for analysis in Model.Analyses:
    	results = analysis.Solution.GetChildren(DataModelObjectCategory.Result, True)
    	for result in results:
    		location ="D:\\"+p[lp]+"\\" + result.Name + ".wmv"
...
path = os.path.join("D:\\", "video")
os.mkdir(path)
for analysis in Model.Analyses:
	results = analysis.Solution.GetChildren(DataModelObjectCategory.Result, True)
	for result in results:
		location ="D:\\video\\" + result.Name + ".wmv"
		result.ExportAnimation(location, GraphicsAnimationExportFormat.WMV, settings)
